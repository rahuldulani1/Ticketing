...
//Users file, handles data transfers to and from SQL for logging in, Registration and data for loading user account page
...
router.post('/register', async (req, res) => {
    ...
    //Validate that all fields are provided in user input
    ...
    try {
        ...
        db.query("SELECT * FROM users WHERE email = ?", [email], async (err, result) => {
            // Check if all fields were enterred by the user
            if (err) {
...
/* User Login
    ...
    If login successful, Index.html displays username on top left
...
/* My Account API
    Navigates user to their account page. Account page displays the existing tickets that user has purchased. 
    First SQL query verifies user exists in DB
    Second query fethches the tickets on their account
*/
router.post('/my-account', async (req, res) => {
    console.log("Incoming /my-account request:", req.body); //debug, check for what API is recieveing 
